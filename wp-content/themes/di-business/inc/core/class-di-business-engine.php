...
class Di_Business_Engine {
	...
	public function scripts_and_styles_admin() {
		wp_enqueue_script( 'di-business-admin-page-metabox-control', get_template_directory_uri() . '/assets/js/admin-page-meta-box-control.js', array( 'jquery' ), DI_BUSINESS_VERSION, true );
	}
	...
	public function sidebar_registration() {
		...
		register_sidebar( array(
			...
			'id'			=> 'sidebar_page',
			'description'	=> __( 'Widgets for Page sidebar. If you are using Full Width No Sidebar Template, then this sidebar will not display.', 'di-business' ),
			'before_widget'	=> '<div id="%1$s" class="widget_sidebar_main clearfix %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="right-widget-title">',
			'after_title'	=> '</h3>',
