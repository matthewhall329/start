...
// Post navigation.
function di_business_post_pagination()
{
			...
			<div class="content-nav-next-pre">
				<nav>
					<ul class="pager">
						...
						{
						?>
							<li class="previous"><?php next_posts_link( __( '&larr; Older Entries ', 'di-business' ) ); ?></li>
						<?php
						}
						?>
						...
						{
						?>
							<li class="next"><?php previous_posts_link( __( 'Newer Entries &rarr;', 'di-business' ) ); ?></li>
						<?php
						}
						?>
...
function di_business_breadcrumbs()
{
    ...
    if ( !is_front_page() )
	{
        ...
        // Home page
        echo '<li><a href="' . esc_url( home_url( '/' ) ) . '">' . __( 'Home', 'di-business' ) . '</a></li>';
           
       if ( is_home() )
	   {
		   echo '<li class="active">' . __( 'Blog', 'di-business' ) . '</li>';
	   }
	   ...
	   elseif( is_archive() && is_tax() && !is_category() && !is_tag() )
	   {
		...
		elseif( is_category() )
		{
            ...
            // Category page
            echo '<li class="active">' . __( 'Category: ', 'di-business' ) . single_cat_title( '', false ) . '</li>';
		...
		elseif( is_tag() )
		{
            ...
            echo '<li class="active">' . __( 'Tag: ', 'di-business' ) . esc_attr( $get_term_name ) . '</li>';
		...
		elseif( is_author() )
		{
            ...
            // Display author name
            echo '<li class="active">' . __( 'Author: ', 'di-business' ) . get_the_author() . '</li>';
           
        }
		elseif( get_query_var( 'paged' ) )
		{
            // Paginated archives
            echo '<li class="active">' . __( 'Page: ', 'di-business' ) . get_query_var( 'paged' ) . '</li>';
        }
		elseif( is_search() )
		{
           
            // Search results page
            echo '<li class="active">' . __( 'Search: ', 'di-business' ) . get_search_query() . '</li>'; 
        }
		elseif( is_404() )
		{
			//error 404 page
            echo '<li class="active">' . __( 'Error 404', 'di-business' ) . '</li>';
        }
		else
		{
			echo '<li class="active">' . __( 'Untitled', 'di-business' ) . '</li>';
		}
...
function di_business_nav_fallback( $args )
{
	...
	$output .= '<li class="menu-item menu-item-type-post_type menu-item-object-page current-menu-item page_item current_page_item active"><a href="' . esc_url( home_url( '/' ) ) . '">'. __( 'Home', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Blog', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Responsive', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'SEO Friendly', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Customizable', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Page Builder', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Typography', 'di-business' ) .'</a></li>';

	$output .= '<li class="menu-item menu-item-type-custom"><a href="#">'. __( 'Forms', 'di-business' ) .'</a></li>';
	
	if( current_user_can( 'manage_options' ) )
	{
		$output .= '<li class="menu-item menu-item-type-custom"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">'. __( 'Add Menu', 'di-business' ) .'</a></li>';
	}
		...
		<div class="comment-author vcard" itemtype="http://schema.org/Comment" itemscope itemprop="comment">
			<div id="comment-<?php comment_ID(); ?>" class="media" >
				...
				<div class="media-body">
					...
					<?php } else { ?>
						<h6 class="media-heading fn"><span itemprop="name"><?php echo esc_attr( get_comment_author() ); ?></span></h6>
					<?php } ?>
