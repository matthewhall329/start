...
function di_business_admin_notice() {
	...
	if( ! get_user_meta( $user_id, 'di_business_ignore_notice' ) ) {
		...
		printf( __( 'Thank you for activating Di Business Theme. Let start form <a target="_blank" href="%1$s">Documentation</a> | <a target="_blank" href="%2$s">Visit Demo</a> | <a target="_blank" href="%3$s">Customize Now</a> | <a href="%4$s">Hide it</a>', 'di-business' ), 'https://dithemes.com/di-business-free-wordpress-theme-documentation/', 'http://demo.dithemes.com/di-business/', esc_url( get_admin_url() . 'customize.php' ) , esc_url( add_query_arg( 'di_business_notics_ignore', '0' ) ) );
...
function di_business_logo_refresh( $wp_customize ) {
	$wp_customize->get_setting( 'custom_logo' )->transport 	= 'refresh';
}
add_action( 'customize_register', 'di_business_logo_refresh' );

// Footer copyright right content
function di_business_footer_copyright_right_setting_front_cntnt() {
	echo '<p>' . __( 'WordPress', 'di-business' ) . ' <a target="_blank" href="https://wordpress.org/themes/di-business/"><span class="fa fa-thumbs-o-up"></span> ' . __( 'Di Business', 'di-business' ) . '</a> ' . __( 'Theme', 'di-business' ) . '</p>';
}
...
function di_business_hdrimg_file_clbk() {
	if( get_the_id() ) {
		if( get_post_meta( get_the_id(), '_di_business_hide_hdrimg', true ) ) {
			return;
		} else {
			if( get_header_image() ) { ?>
